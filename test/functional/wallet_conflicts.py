...
class TxConflicts(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Create conflict by broadcast conflicted tx on node0 chain and conflicting txn on node1 chain
        conflicting_txid_A = self.nodes[1].sendrawtransaction(conflicting_tx_A["hex"])
        self.nodes[1].generate(4)
        conflicting_txid_B = self.nodes[1].sendrawtransaction(conflicting_tx_B["hex"])
        self.nodes[1].generate(4)
        ...
        assert_equal(conflicted["confirmations"], -8)
        assert_equal(conflicted["status"], "CONFLICTED")
        assert_equal(child_conflicted["status"], "CONFLICTED")

        # Node2 chain without conflicts
        nw = self.nodes[2].sendrawtransaction(conflicted["hex"])
        ...
        conflicted = self.nodes[0].gettransaction(conflicted_txid)
        child_conflicted = self.nodes[0].gettransaction(child_conflicted_txid)
        assert_equal(conflicted["status"], "CONFIRMED")
        #TODO: child shoudn't be anymore conflicted as conflict as been removed from parent
        assert_equal(child_conflicted["status"], "CONFLICTED")
