...
class CoinpoolTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Internal key updpates checkpoints
        ...
        alice_withdraw_tx.vin.append(CTxIn(COutPoint(int(setup_txid, 16), 0), b"", 0xffffffff))
        ...
        # Cancel Alice contribution from the aggregated key
        Pna = SECP256K1.negate(Pa)
        ...
        # Construct the post-alice-withdraw version of the pool tree
        # Alice point and leaf have been pruned out
        ...
        alice_pool_output = CTxOut(bob_amount + caroll_amount + dave_amount, alice_pool_tree[0])
        alice_withdraw_tx.vout.append(alice_pool_output)
        # Sign the withdraw transaction with `SIGHASH_ANYPREVOUT | SIGHASH_GROUP | SIGHASH_ANYPUBKEY | SIGHASH_ANYAMOUNT`
        # ANNEX_TAG + ANNEX_GROUP + length-field + group_count + ANNEX_ANYPUBKEY + length-field + anypubkey-tagged outputs + ANNEX_ANYAMOUNT + length-field + anyamount-tagged outputs
        ...
        # Broadcast Alice withdraw transaction spending the setup output
        ...
        # Construct the post-bob-withdraw version of the pool tree
        # Bob point and leaf have been pruned out
        ...
        bob_withdraw_hash = TaprootSignatureHash(bob_withdraw_tx, [alice_pool_output], bob_withdraw_hashtype, 0, True, script=bob_withdraw_tapscript, annex=alice_annex, leaf_ver=TAPROOT_LEAF_WITHPARITY, key_ver=1, group_outputs=[0, 1], group_anypubkey=[0, 1], group_anyamount=[0, 1])
        ...
        # Broadcast Bob withdraw transaction spending Alice withdraw tx `pool_output`
        ...
        # Consruct the post-caroll-withdraw version of the pool tree
        # Caroll point and leaf have been pruned out
        ...
        # Broadcast Caroll withdraw transaction spending Bob withdraw tx `pool_output`
        ...
        # There is only Dave point remaining in the internal pubkey
        ...
        # Broadcast Dave withdraw transaction spending Caroll withdraw tx `pool_output`
