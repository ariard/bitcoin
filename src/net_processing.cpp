...
/** How long to delay requesting transactions from overloaded peers (see MAX_PEER_TX_IN_FLIGHT). */
...
void PeerManager::AddTxAnnouncement(const CNode& node, const GenTxid& gtxid, std::chrono::microseconds current_time)
{
    ...
    //     MAX_PEER_TX_IN_FLIGHT requests in flight.
    auto delay = std::chrono::microseconds{0};
    const bool preferred = state->fPreferredDownload;
    if (!preferred) delay += NONPREF_PEER_TX_DELAY;
    if (!state->m_wtxid_relay && g_wtxid_relay_peers > 0) delay += TXID_RELAY_DELAY;
    const bool overloaded = m_txrequest.CountInFlight(nodeid) >= MAX_PEER_TX_IN_FLIGHT;
