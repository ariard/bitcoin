...
bool CWallet::AddToWalletIfInvolvingMe(const CTransactionRef& ptx, CWalletTx::Status status, const uint256& block_hash, int posInBlock, bool fUpdate, int block_height)
{
    const CTransaction& tx = *ptx;
    {
        ...
        if (fExisted || IsMine(tx) || IsFromMe(tx))
        {
            ...
            wtx.SetConf(status, block_hash, posInBlock, block_height);
...
void CWallet::SyncTransaction(const CTransactionRef& ptx, CWalletTx::Status status, const uint256& block_hash, int block_height, int posInBlock, bool update_tx)
{
    if (!AddToWalletIfInvolvingMe(ptx, status, block_hash, posInBlock, update_tx, block_height))
...
void CWallet::BlockDisconnected(const CBlock& block, int height)
{
    ...
    for (const CTransactionRef& ptx : block.vtx) {
        SyncTransaction(ptx, CWalletTx::Status::UNCONFIRMED, {} /* block hash */, 0 /* block height */, 0 /* position in block */);
    }
}
    ...
    while (block_height && !fAbortRescan && !chain().shutdownRequested()) {
        ...
        if (chain().findBlock(block_hash, &block) && !block.IsNull()) {
            ...
            for (size_t posInBlock = 0; posInBlock < block.vtx.size(); ++posInBlock) {
                SyncTransaction(block.vtx[posInBlock], CWalletTx::Status::CONFIRMED, block_hash, *block_height, posInBlock, fUpdate);
            }
