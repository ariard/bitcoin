...
void LegacyScriptPubKeyMan::MarkUnusedAddresses(const CScript& script)
{
    ...
    for (const auto& keyid : GetAffectedKeys(script, *this)) {
        ...
        if (it != mapKeyMetadata.end()){
            ...
            if (!meta.hd_seed_id.IsNull() && meta.hd_seed_id != hdChain.seed_id) {
                ...
                if (!FillInactiveChain(meta.hd_seed_id, index, internal)) {
...
void LegacyScriptPubKeyMan::DeriveNewChildKey(WalletBatch &batch, CKeyMetadata& metadata, CKey& secret, CHDChain& hd_chain, bool internal)
{
    ...
    if (hd_chain.seed_id == hdChain.seed_id) {
...
bool LegacyScriptPubKeyMan::TopUp(unsigned int kpSize)
{
    ...
    }
    {
        ...
        for (int64_t i = missingInternal + missingExternal; i--;)
        {
            ...
            CPubKey pubkey(GenerateNewKey(batch, hdChain, internal));
            AddKeypoolPubkeyWithDB(pubkey, internal, batch);
        }
...
bool LegacyScriptPubKeyMan::GetKeyFromPool(CPubKey& result, const OutputType type, bool internal)
{
    ...
    CKeyPool keypool;
    {
        ...
        if (!ReserveKeyFromKeyPool(nIndex, keypool, internal) && !m_storage.IsWalletFlagSet(WALLET_FLAG_DISABLE_PRIVATE_KEYS)) {
            if (m_storage.IsLocked()) return false;
            WalletBatch batch(m_storage.GetDatabase());
            result = GenerateNewKey(batch, hdChain, internal);
            return true;
        }
