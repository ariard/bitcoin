...
bool SignatureHashSchnorr(uint256& hash_out, const ScriptExecutionData& execdata, const T& tx_to, uint32_t in_pos, uint8_t hash_type, SigVersion sigversion, KeyVersion keyversion, const PrecomputedTransactionData& cache, MissingDataBehavior mdb)
{
    ...
    if ((output_type & SIGHASH_GROUP) == SIGHASH_GROUP) {
        // Specification: If `start` is equal to `end or `end` is superior to the number of outputs,
        // the validation MUST be failed.
        if (execdata.m_group->first == execdata.m_group->second || execdata.m_group->second >= tx_to.vout.size()) return false;

        // Specification: If the value committed of the group outputs amounts is NOT equal to the
        // input amount one.
        if (VerifyOutputsGroup(tx_to, cache.m_spent_outputs[in_pos].nValue, execdata.m_group->first, execdata.m_group->second)) return false;
        ...
        for (unsigned int out_pos = execdata.m_group->first; out_pos < execdata.m_group->second + 1; out_pos++) {
            ...
            // Specification: If an output has a corresponding bit set in the ANYPUBKEY bitmap,
            // the `scriptPubKey` must be omitted from the hash.
            if (!anypubkey_flag) {
                ss << tx_to.vout[out_pos].scriptPubKey;
            }
            // Specification: If the output has a corresponding bit set in the ANYAMOUNT bitmap,
...
bool GenericTransactionSignatureChecker<T>::CheckMerkleUpdate(const std::vector<unsigned char>& control, unsigned int out_pos, const std::vector<unsigned char>& point) const
{
    ...
    try {
        // Specification: The "update public key" is subtracted from the internal pubkey, the result
        // is interpreted as the "modified internal pubkey I".
    ...
    // now due to OP_SUCCESS integration, shrug about the 33-byte or only 1 control node case...)
    ...
    // Specification: if `output_pos` is superior to the number of outputs in the spending transaction,
    // the script MUST fail and terminate immediately.
    if (out_pos >= txTo->vout.size()) return false;
    int witnessversion;
    std::vector<unsigned char> witnessprogram;
    // Specification: The output at position `output_pos` in the spending transaction is interpreted
    // as the "destination output".
    if (!txTo->vout[out_pos].scriptPubKey.IsWitnessProgram(witnessversion, witnessprogram)) {
        // Specification: If the "destination output" is NOT a witness version 1 output, the script
        // MUST fail and terminate immediately.
        return false;
    }
    // If the "destination output" is NOT a witness version 1 output, the script MUST fail and
    // MUST fail and terminate immediately.
    if (witnessversion == 0) {
        return false;
    } else if (witnessversion == 1) {
        const XOnlyPubKey q{uint256(witnessprogram)};
        // Specification: The new merkle root "t" is re-computed from the "tapleaf hash" and the
        // "control block" as defined in BIP341.
        const uint256 merkle_root = ComputeTaprootMerkleRoot(control, updated_tapleaf_hash, 1);
        // Specification: Let Q = U + t^G as defined in BIP341. If the "destination output" witness
        // program is NOT equal to "Q", the script MUST fail and terminate immediately.
...
static bool VerifyAnnex(const std::vector<unsigned char>& annex, ScriptExecutionData& execdata)
{
    ...
    for (int i = 1; i < annex_len;) {
        ...
        switch (record_type)
        {
            ...
            {
                group_present = true;
                // Specification: The ANNEX_GROUP value record is called the `group_count` and
                // interpreted as a VarInt.
                ...
                if (group_count < 0) {
                    ...
                    // Specification: If the ANNEX_GROUP field is present and `group_count` is
                    // superior to 0, set `start` equal to `end` then `end` equal to `start` +
                    // `group_count`.
                    execdata.m_group->first = execdata.m_group->second;
                    execdata.m_group->second += group_count;
                }
                break;
            }
            case ANNEX_ANYPUBKEY:
            {
                // Specification: If the ANNEX_ANYPUBKEY field is present and the ANNEX_GROUP field
                // is absent. If NOT, the validation must be failed.
                if (!group_present) {
                    return false;
                }
                // Specification: The ANNEX_ANYPUBKEY value record is interpreted as a bitmap, where
                // a set bit stands for the corresponding output at the same index opt-ing out from
                // `scriptPubKey` signature digest commitment.
                for (int j=0; j < record_length; j++) {
                    unsigned int byte = annex[i + 2 + j];
                    for (int k=0; k < 8; k++) {
                        unsigned int out_pos = execdata.m_group->first + j * 7 + k;
            ...
            {
                // Specification: If the ANNEX_ANYAMOUNT field is present and the ANNEX_GROUP field
                // is absent. If NOT, the validation must be failed.
                if (!group_present) {
                    return false;
                }
                // Specification: The ANNEX_ANYAMOUNT value record is intepreted as a bitmap, where
                // a set bit stands for the corresponding output at the same index opt-ing out from
                // `amount` signature digest commitment.
                for (int j=0; j < record_length; j++) {
                    unsigned int byte = annex[i + 2 + j];
                    for (int k=0; k < 8; k++) {
                        unsigned int out_pos = execdata.m_group->first + j * 7 + k;
    ...
    if (!group_present) {
        // Specification: If the ANNEX_GROUP is absent in the annex, set start equal to end
        execdata.m_group->first = execdata.m_group->second;
    }
    return true;
}
...
static bool VerifyTaprootCommitment(const std::vector<unsigned char>& control, const std::vector<unsigned char>& program, const uint256& tapleaf_hash, std::optional<XOnlyPubKey>* internal_key, std::optional<std::vector<unsigned char>>* control_block)
{
    ...
    bool ret;
    // If the leaf is v2, the internal pubkey parity bit marker is commited in the tweak hash
