...
enum class ThresholdState {
    DEFINED,   // First state that each softfork starts out as. The genesis block is by definition in its state for each deployment.
    STARTED,   // For blocks past the starttime.
    LOCKED_IN, // For one retarget period after the first retarget period with STARTED blocks of which at least threshold have the associated bit set in nVersion.
    ACTIVE,    // For all blocks after the LOCKED_IN retarget period (final state)
    FAILED,    // For one retarget period past the timeout time, if LOCKED_IN was not reached (final state)
};
