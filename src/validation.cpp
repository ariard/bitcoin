...
bool CScriptCheck::operator()() {
    ...
    return VerifyScript(scriptSig, m_tx_out.scriptPubKey, witness, nFlags, CachingTransactionSignatureChecker(ptxTo, nIn, m_tx_out.nValue, cacheStore, *txdata), group, &error);
}
                       ...
                       std::vector<CScriptCheck>* pvChecks)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++) {
        ...
        // Verify signature
        CScriptCheck check(txdata.m_spent_outputs[i], tx, i, flags, cacheSigStore, &txdata, &group);
        if (pvChecks) {
            ...
            if (flags & STANDARD_NOT_MANDATORY_VERIFY_FLAGS) {
                ...
                CScriptCheck check2(txdata.m_spent_outputs[i], tx, i,
                        flags & ~STANDARD_NOT_MANDATORY_VERIFY_FLAGS, cacheSigStore, &txdata, &group);
                if (check2())
