 ...
 *
 * It includes a naive reimplementation of its behavior, for a limited set of MAX_TXHASHES distinct txids, and
 * MAX_PEERS peer identifiers.
 *
 * All of the public member functions perform the same operation on an actual TxRequestTracker and on the state of
 * the reimplementation. The output of GetRequestable is compared with the expected value as well.
 *
 * Check() calls the TxRequestTracker's sanity check, plus compares the output of the constant accessors (Size(),
 * Count(), CountInFlight(), CountCandidates()) with expected values.
 */
class Tester
{
