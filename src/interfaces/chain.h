...
public:
    ...
    public:
        ...
        virtual void BlockConnected(const CBlock& block, const std::vector<CTransactionRef>& tx_conflicted, int height, FlatFilePos block_pos) {}
    ...
    //! Start notifications sequence in order for every ChainClient, XXX:should only be called by init code.
    virtual void startNotifications() = 0;

    //! Stop notifications sequence, XXX:should only be called by stop code.
    virtual void stopNotifications() = 0;

    //! Interrupt notification sequence, XXX: should only be called by stop code.
